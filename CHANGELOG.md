...
- `activeToolCap` bounds simultaneously active search-activated direct tools (default 24), evicting the least-recently-used unused tool first; a successful `mcp({ tool })` call also activates a held search-mode tool; activation is reconciled from the transcript at session start and on branch navigation.
