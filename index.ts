...
function installMcpAdapter(pi: ExtensionAPI, options: McpAdapterOptions) {
    ...
    callReentrant(() => (pi.registerTool as (tool: unknown) => unknown)({
      ...
      async execute(toolCallId: string, params: Record<string, unknown>, signal: AbortSignal | undefined, onUpdate: AgentToolUpdateCallback<Record<string, unknown>> | undefined, ctx: ExtensionContext) {
        ...
        try {
          ...
          // Search-mode tools count toward the active cap and are evicted least-recently-
          // used; a use here keeps the slot. Eager tools live outside the cap.
          if (spec.lazy) executor = withUsageTracking(spec.prefixedName, executor);
        } catch (error) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    searchActivatedTools.length = 0;
    usedSearchTools.clear();
    holdLazyToolsInactive();
    const generation = ++lifecycleGeneration;
    ...
    callReentrant(() => (pi.registerTool as (tool: unknown) => unknown)({
      ...
      async execute(_toolCallId: string, params: {
        ...
        if (params.tool) {
          ...
          const { added, evicted, capped, cap } = activateSearchMatches(proxyState.config, [{ server: identity.server, tool: name }]);
